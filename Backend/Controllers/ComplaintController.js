...
const addComplaints = async (req, res, next) =>{
    const{complaintid,serviceName,serviceProvider,date,location, bookingId, complaintCategory, urgencyLevel}=req.body;

    let complaints;
    try{
        complaints = new complaint({complaintid,serviceName,serviceProvider,date,location, bookingId, complaintCategory, urgencyLevel});
...
const updateComplaint = async (req, res, nest)=>{
    ...
    try{
        Complaints = await complaint.findByIdAndUpdate(id,
            {complaintid: complaintid,serviceName: serviceName ,serviceProvider: serviceProvider,date: date,location: location, bookingId: bookingId, complaintCategory: complaintCategory, urgencyLevel: urgencyLevel}
        , { new: true});
